...
- 1997922: Fix secondary review approval handoff and durable review persistence:
...
- 0d32dd3: Encourage open models, Gemini, and Claude to actively utilize Workflow Guard features:
...
- 5c09afb: Bound compaction continuity context with operational-state-first priority and project-memory retrieval guidance.
...
- 42d5aef: Improve multi-model workflow adherence by enriching mutating tool definitions (`edit`, `write`, `apply_patch`) with operational preconditions, grounding synthetic continuation prompts with remaining active tasks, and adding in-context circuit breaker steering on repeated blocked operations.

## 1.9.3

### Patch Changes

- 8008205: Fix releases failing after publish when the newly created Git tag already exists remotely.
...
- 960a59f: Block PR descriptions with literal newline escapes that would render as text, direct callers to shell-safe multiline forms, and load SQLite through Bun when running inside OpenCode so durable guard state initializes correctly.
